...
*TODO: THIS IS UNTESTED ON FUNCTIONING iOS CODE*

=== Today Screen Widgets

We'll make an extension that lists your three most recently edited notes on the Today screen, and lets you jump straight to them

. File->New->Target
    . iOS -> Application Extension -> Today extension
    . Product name = Notes-TodayScreen
    . Embed In: Notes-iOS
    . Click finish, Activate scheme when prompted

First let's learn how to run extensions:

. Run the target (⌘-R)
. Xcode will provide a list of applications that it should try to launch and use to get to the extension
    . Because this is a Today extension, it will suggest launching the Today app (that's the built-in app that appears when you swipe down from the top of the screen)
    . Click Run
    . The phone will launch
    . You'll see a console error about constraints - this is an error in the template, and we'll be fixing it *TODO: CHECK THIS BEFORE RELEASE TO ENSURE THAT APPLE HASN'T FIXED IT IN THE MEANTIME*
    
. Delete the Hello World label
...
. Add a table view
    . Add the following constraints:
        ...
        . Set the priority of all these constraints to 750
    
    . Update frame of table view (select it, open triangle tie fighter, Update Frames)
    . Set content to dynamic prototypes
    . Set number of prototypes to 1
. Select the prototype cell
    . Set style to basic
    . Select the text label
        . Set color to Light Text Color
        . Set font to System Light 16.0

. Connect the table view to the view controller, make it its delegate and data source

. Open TodayViewController.swift
    . Conform to UITableViewDelegate and UITableViewDatasource
    . Implement number of rows = 3 and cell for row = returns cell with identifier "Cell"
    . Run the app, you'll get rows
    
Now we need the extension to be able to view the documents

. Go to Notes-TodayScreen target's settings, go to Capabilities
. Turn on iCloud
. Turn on iCloud Documents
. Change containers option to Specify Custom Containers, turn on the same container as the iOS app (and no others)

The extension can now access the iOS app's container!

. Implement +TodayViewController+'s +fileList+ property
. Implement +loadAvailableFiles+ to search for and return the list of documents
. Implement +viewDidLoad+ to call +loadAvailableFiles+ and also establish access to the container
. Implement the table view data source methods to return the list
. Implement +widgetPerformUpdateWithCompletionHandler+ to re-run the check and to indicate whether the file list has changed or not

Run the extension - a list of documents appears!

Now we need to make the app open when a document is selected

. Go to Notes-iOS's project, go to Info tab, scroll down to URL Type
. Add a new one, scheme = "notes"

. Open Notes-iOS ViewController.swift
    . Implement +openDocumentWithPath+, which takes a path and attempts to open it
. Open AppDelegate.swift
    . Implement openURL to receive the URL and tell the document list controller to open the doc

. Open TodayViewController
    . Implement +didSelectRowAtIndexPath+ to open the URL
    
Run the extension again, tap a document; it opens!
